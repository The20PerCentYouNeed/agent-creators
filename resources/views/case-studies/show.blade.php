...
@section('content')
    ...
    {{-- Featured Image --}}
    @if(isset($caseStudy['image_url']) || isset($caseStudy['logo_url']))
    <section class="py-12 px-4 sm:px-6 lg:px-8 bg-white dark:bg-gray-950">
        <div class="max-w-7xl mx-auto">
            @if(isset($caseStudy['image_url']) && $caseStudy['image_url'])
            <div class="w-full h-96 bg-gradient-to-br {{ $caseStudy['gradient'] ?? 'from-blue-600 to-violet-600' }} rounded-2xl overflow-hidden">
                <img src="{{ $caseStudy['image_url'] }}" alt="{{ $caseStudy['title'] }}" class="w-full h-full object-cover">
            </div>
            @else
            <div class="w-full h-96 bg-gradient-to-br {{ $caseStudy['gradient'] ?? 'from-blue-600 to-violet-600' }} rounded-2xl flex items-center justify-center">
    ...
    {{-- Full Description --}}
    @if(isset($caseStudy['full_description']) && $caseStudy['full_description'])
    ...
    {{-- Solution --}}
    @if(isset($caseStudy['solution']) && $caseStudy['solution'])
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="prose prose-lg dark:prose-invert max-w-none">
                <div class="text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-line">
                    {!! \Illuminate\Support\Str::markdown($caseStudy['solution'] ?? '') !!}
                </div>
            </div>
        </div>
    </section>
